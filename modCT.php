<?php
  session_start();
	...
	$sql = "SELECT id_usuario,id_coorTerapeuta, nombre, apellido_paterno, apellido_materno, facultad_origen, telefono, celular, correo_personal FROM coordinador_terapeuta INNER JOIN usuario WHERE id_coorTerapeuta='$id' AND id_usuarioFK=id_usuario;";
...
<body>
    ...
    <div class="bg-agile">
        <div class="book-appointment">
            <h1> MODIFICAR COORDINADOR TERAPEUTAS</h1>

            <form action="updateCT.php" method="POST" name="h">

                <div id="resultados_ajax" class="gaps"></div>
                <div class="left-agileits-w3layouts same">

                    <input type="hidden" id="id" name="idCoorTera" value="<?php echo $row['id_coorTerapeuta']; ?>" />
                    <input type="hidden" id="id_u" name="idUsuario" value="<?php echo $row['id_usuario']; ?>" />


                    <div class="gaps form-group">
                        <p>Nombre(s):</p>
                        <input onkeyup="limpiarletras(this)" onchange="limpiarletras(this)" type="text" name="nombreCT"
                            pattern="([a-zA-Z]*+[ ]+[a-zA-Z])" value="<?php echo $row['nombre']; ?>" required
                            onpaste="return false" />
                    </div>
                    <div class="gaps form-group">
                        <p>Apellido Paterno:</p>
                        <input onkeyup="limpiarletras(this)" onchange="limpiarletras(this)" type="text"
                            name="apellidoPaCT" pattern="([a-zA-Z]*+[ ]+[a-zA-Z])" value="<?php echo $row['apellido_paterno']; ?>"
                            required onpaste="return false" />
                    </div>
                    <div class="gaps form-group">
                        <p>Apellido Materno:</p>
                        <input onkeyup="limpiarletras(this)" onchange="limpiarletras(this)" type="text"
                            name="apellidoMaCT" pattern="([a-zA-Z]*+[ ]+[a-zA-Z])" value="<?php echo $row['apellido_materno']; ?>"
                            required onpaste="return false" />
                    </div>
                    <div class="gaps form-group">
                ...
                <div class="right-agileinfo same">
                    <div class="gaps form-group">
                        <p>Facultad:</p>
                        <select class="form-control" name="facultadCT" required>
                            <option disabled selected>seleciona tu facultad </option>
                            <option value="Ingenieria"
                                <?php if($row['facultad_origen']=='Ingenieria') echo 'selected'; ?>>Ingenieria
                            </option>
                            <option value="Derecho" <?php if($row['facultad_origen']=='Derecho') echo 'selected'; ?>>
                                Derecho
                            </option>
                    ...
                    <div class="gaps form-group">
                        ...
                        <input onkeyup="limpiarNumero(this)" onchange="limpiarNumero(this)" type="tel" name="telefonoCT"
                            MAXLENGTH=10 pattern="[0-9]{10}" value="<?php echo $row['telefono']; ?>" required
                            onpaste="return false" />
                    </div>
                    <div class="gaps form-group">
                        <p>Número de celular</p>
                        <input onkeyup="limpiarNumero(this)" onchange="limpiarNumero(this)" type="tel" name="celularCT"
    ...
    <div class="book-appointment">
        <input type="submit" onclick="location.href='tablaModCT.php'" value="CANCELAR" style="   margin-left: 10px  ">
    </div>
    </div>
