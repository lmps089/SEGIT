...
function CoordinadorExisteN($nombre){
  ...
  if($stmt = $mysqli->prepare('SELECT nombre FROM coordinador_terapeuta WHERE nombre LIKE ? LIMIT 1')){
...
function CoordinadorExisteAP($apellidoPaterno){
    ...
    if($stmt = $mysqli->prepare('SELECT apellido_paterno FROM coordinador_terapeuta WHERE apellido_paterno LIKE ? LIMIT 1')){
  ...
  function CoordinadorExisteAM($apellidoMaterno){
      ...
      if($stmt = $mysqli->prepare('SELECT apellido_materno FROM coordinador_terapeuta WHERE apellido_materno LIKE ? LIMIT 1')){
...
function registraCoordinador($nombre,$apellidoMaterno,$apellidoPaterno, $telefono, $celular, $facultad){
  ...
  if($stmt = $mysqli->prepare("INSERT INTO coordinador_terapeuta(nombre , apellido_materno, apellido_paterno, telefono,  	celular, facultad_origen ,id_usuarioFK) VALUES(?,?,?,?,?,?,(SELECT id_usuario FROM usuario order by id_usuario desc limit 1))")){
...
function eliminaCoordTerapeuta($id){
  ///esta funcion elimina al coordinador de Terapeuta y a su usuario, que registro cuando se dio de alta;
    global $mysqli;
    //se obtiene el id del su usuario
    if($stmt = $mysqli->prepare("SELECT id_usuarioFK FROM coordinador_terapeuta WHERE id_coorTerapeuta=?")){
      ...
      if($stmt->execute()){// si se realiza la consulta
        ...
        $stmt->fetch();
        //se elimina el coordinador de Terapeuta
        $stmt1 = $mysqli->prepare("DELETE FROM coordinador_terapeuta where id_coorTerapeuta=?");
          $stmt1->bind_param('i',$id);
          
          if($stmt1->execute() ){//si se elimina el coordinador
            echo 'el id es:'.$id;
          //se elimina su usuario del coordinador de Terapeuta
          $stmt2 = $mysqli->prepare("DELETE FROM usuario where id_usuario=?");
          $stmt2->bind_param('i',$idUsuarioFK);
            if($stmt2->execute()){// si se elimina el usuario del coordinador de Terapeuta
                return true;
...
function modificarCT($id,$nombre,$apellidoMaterno,$apellidoPaterno,$telefono,$celular,$facultad){
  global $mysqli;

  if($stmt = $mysqli->prepare('UPDATE coordinador_terapeuta SET nombre=?,apellido_materno=?,apellido_paterno=?,telefono=?,celular=?,facultad_origen=? WHERE  	id_coorTerapeuta=?')){
...
function modificarCT2($id,$nombre,$apellidoMaterno,$apellidoPaterno,$telefono,$celular,$facultad,$correoPernsonal,$idUsuario){
  global $mysqli;
